...
{
    public abstract class ExchangeWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ExchangeWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ExchangeWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<ExchangeDbContext> action)
        {
            using (var context = IocManager.Resolve<ExchangeDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<ExchangeDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<ExchangeDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ExchangeDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ExchangeDbContext, Task<T>> func)
        {
            T result;
