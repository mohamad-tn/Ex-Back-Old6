...
using Bwr.Exchange.Controllers.Statements;
using Bwr.Exchange.Models.Statments.Totals;

namespace Bwr.Exchange.Controllers
{
    [Route("api/[controller]/[action]")]
    public class PdfCompanyController : PdfStatmentControllerBase
    {
        private readonly ICompanyCashFlowAppService _companyCashFlowAppService;

        public PdfCompanyController(ICompanyCashFlowAppService companyCashFlowAppService)
        {
            _companyCashFlowAppService = companyCashFlowAppService;
        }

        [HttpPost]
        public PdfResultOutput GetTotalCompanyBalance(GetTotalCompanyBalanceInput input)
        {
            var dateStr = DateTime.Now.ToString("dd-MM-yyyy-HH.mm");
            var fileName = $"company-balance_{dateStr}";
            var path = $"wwwroot/statments/{fileName}.pdf";

            var currencies = InitialCurrencies(input.TotalCompanyBalancePdfs);
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    {
                        column.Item().AlignCenter().PaddingHorizontal(10).Text("كشف ارصدة الشركات").SemiBold().FontSize(20).FontFamily(Fonts.Calibri);
                        column.Item().Row(row =>
                        {
                    ...
                    .BorderColor(Colors.Grey.Lighten1)
                        .Table(table =>
                        {
                            ...
                            foreach (var item in input.TotalCompanyBalancePdfs)
                            {
                                table.Cell().Element(CellStyle).Text(item.CompanyName).FontFamily(Fonts.Calibri);
                            ...
                            table.Footer(footer =>
                            {
                                var forHim0 = input.TotalCompanyBalancePdfs.Where(x => x.Balance0 < 0).Sum(x => x.Balance0);
                                var forHim1 = input.TotalCompanyBalancePdfs.Where(x => x.Balance1 < 0).Sum(x => x.Balance1);
                                var forHim2 = input.TotalCompanyBalancePdfs.Where(x => x.Balance2 < 0).Sum(x => x.Balance2);
                                var forHim3 = input.TotalCompanyBalancePdfs.Where(x => x.Balance3 < 0).Sum(x => x.Balance3);
                                var forHim4 = input.TotalCompanyBalancePdfs.Where(x => x.Balance4 < 0).Sum(x => x.Balance4);

                                var onHim0 = input.TotalCompanyBalancePdfs.Where(x => x.Balance0 > 0).Sum(x => x.Balance0);
                                var onHim1 = input.TotalCompanyBalancePdfs.Where(x => x.Balance1 > 0).Sum(x => x.Balance1);
                                var onHim2 = input.TotalCompanyBalancePdfs.Where(x => x.Balance2 > 0).Sum(x => x.Balance2);
                                var onHim3 = input.TotalCompanyBalancePdfs.Where(x => x.Balance3 > 0).Sum(x => x.Balance3);
                                var onHim4 = input.TotalCompanyBalancePdfs.Where(x => x.Balance4 > 0).Sum(x => x.Balance4);
        ...
        [HttpPost]
        public PdfResultOutput GetCompanyCashFlow(GetCompanyCashFlowPdfInput input)
        {
            ...
            dm.id = input.CompanyId;

            var companyCashFlows = (IEnumerable<CompanyCashFlowDto>)_companyCashFlowAppService.GetForGrid(dm).result;

            var dateStr = DateTime.Now.ToString("dd-MM-yyyy HH.mm");
            var fileName = $"company-cash-flow_{dateStr}";
            var path = $"wwwroot/statments/{fileName}.pdf";

            var lastCashFlow = companyCashFlows.LastOrDefault();
                ...
                container.Page(page =>
                {
                    ...
                    page.Header().AlignCenter().Padding(10).Column(column =>
                    {
                        ...
                        column.Item().Row(row =>
                        {
                            ...
                            row.ConstantItem(150).Element(ParamsValueStyle).Text(input.CompanyName).FontFamily(Fonts.Calibri);
                            //Space
                    ...
                    .BorderColor(Colors.Grey.Lighten1)
                        .Table(table =>
                        {
                            ...
                            foreach (var item in companyCashFlows)
                            {
                                //الرصيد
                                string balance = "";
                                if (item.Balance != 0)
                                ...
                                //العمولة
                                var commission = item.Commission > 0 ? item.Commission : item.CompanyCommission;
        ...
        private List<string> InitialCurrencies(IList<TotalCompanyBalancePdf> totalCompanyBalances)
        {
            var currencies = new List<string>();
            TotalCompanyBalancePdf companyBalancePdf = null;
            companyBalancePdf = totalCompanyBalances.FirstOrDefault(x => !string.IsNullOrEmpty(x.Currency0));
            if (companyBalancePdf != null)
                currencies.Add(companyBalancePdf.Currency0);

            companyBalancePdf = totalCompanyBalances.FirstOrDefault(x => !string.IsNullOrEmpty(x.Currency1));
            if (companyBalancePdf != null)
                currencies.Add(companyBalancePdf.Currency1);

            companyBalancePdf = totalCompanyBalances.FirstOrDefault(x => !string.IsNullOrEmpty(x.Currency2));
            if (companyBalancePdf != null)
                currencies.Add(companyBalancePdf.Currency2);

            companyBalancePdf = totalCompanyBalances.FirstOrDefault(x => !string.IsNullOrEmpty(x.Currency3));
            if (companyBalancePdf != null)
                currencies.Add(companyBalancePdf.Currency3);

            companyBalancePdf = totalCompanyBalances.FirstOrDefault(x => !string.IsNullOrEmpty(x.Currency4));
            if (companyBalancePdf != null)
                currencies.Add(companyBalancePdf.Currency4);
